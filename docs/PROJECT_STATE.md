...
## M11 Notifications — templates + dispatch + detectors + sender LIVE (keys pending)
...
- Sender (edge fn notification-sender, N4): service-role withSupabase auth:["secret"] (unauth→401); drains PENDING notifications(SMS/EMAIL/WHATSAPP, PUSH excluded)+communication_logs+report_deliveries via Twilio+SendGrid, marks SENT/FAILED+sent_at. Deployed, inert until keys. TRIGGER DEFERRED (pg_net */2 cron would FAIL rows pre-keys; pg_net not installed). Keys: supabase secrets set TWILIO_*/SENDGRID_KEY/FROM_EMAIL. Real-send gate deferred to key-set. In-app UI = NotificationsPage via /inventory/notifications bell; no push pkg.
NEXT: set provider keys + activate drain cron; notification_preferences seed; global inbox; migrate producers to notify().
