...
## M11 Notifications — templates + dispatch + detectors + sender + push-store LIVE (keys pending)
...
- Detectors (notifications_detectors + fn_overdue_receivables_date_fix): fn_overdue_receivables / fn_unpaid_salaries / fn_stock_mismatch → daily-idempotent IN_APP HIGH/URGENT admin alerts; pg_cron 07:00 (notif_detectors_daily, MANUAL). Joins fn_low_stock_notify. Gate-proven rolled-back. (Fix: overdue predicate was interval>int → 42883 every call; compare day-counts.)
- Sender (edge fn notification-sender, N4+N5): service-role withSupabase auth:["secret"] (unauth→401); drains PENDING notifications(SMS/EMAIL/WHATSAPP + PUSH→device_tokens)+communication_logs+report_deliveries via Twilio+SendGrid+FCM v1, marks SENT/FAILED. Deployed, inert until keys. TRIGGER DEFERRED (pg_net */2 cron would FAIL rows pre-keys; pg_net not installed). Keys: supabase secrets set TWILIO_*/SENDGRID_KEY/FROM_EMAIL/FCM_SERVICE_ACCOUNT.
- Push (notifications_device_tokens, SCHEMA EXT signed off): device_tokens + register_device_token (upsert, RLS own). Gate-proven. FLUTTER firebase_messaging DEFERRED (needs Firebase project + platform config; would break build). In-app UI = NotificationsPage via /inventory/notifications bell. NEXT: set provider keys + activate drain cron; notification_preferences seed; global inbox; migrate producers to notify().
