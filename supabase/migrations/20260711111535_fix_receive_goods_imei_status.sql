-- receive_goods captured serialized IMEIs with status 'IN_STOCK', which is not a
-- value of imei_status_enum (AVAILABLE/SOLD/RETURNED/TRANSFERRED/SCRAPPED/RESERVED/
-- IN_TRANSIT) → every serialized receipt raised 22P02. Use 'AVAILABLE' (the in-stock
-- value; matches how sales marks an IMEI SOLD off AVAILABLE). Forward CREATE OR REPLACE,
-- idempotent, zero data change. Body otherwise identical to 20260711101802.
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    -- IMEI capture for serialized products (imei_ids present) -> imei_records AVAILABLE
      ...
      for v_imei in select jsonb_array_elements_text(v_item->'imei_ids') loop
        if length(trim(v_imei)) > 0 then
          ...
          values (v_t, v_po_item.product_id, v_po_item.variant_id, trim(v_imei),
            'AVAILABLE'::imei_status_enum, v_branch, null, 'GRN', v_grn_id, v_cost_unit)
