-- ⚠️ Load-bearing fact #4: `create or replace` with a DIFFERENT arg list creates an OVERLOAD, not a
-- replacement — then every live 6-arg call from Flutter dies on "function create_sale(...) is not unique".
-- drop+create IN ONE FILE is the only correct form. supabase db push wraps each file in a transaction, so
-- there is NO window where create_sale does not exist. DO NOT SPLIT THIS ACROSS TWO MIGRATIONS.
drop function if exists public.create_sale(uuid, uuid, jsonb, jsonb, text, uuid);
...
create function public.create_sale(
  ...
  p_idempotency_key uuid default null      -- NEW. Defaulted -> existing 6-arg callers unchanged.
...
-- Restore the original hardened ACL: `create function` re-grants EXECUTE to PUBLIC by default, but the live
-- create_sale never allowed PUBLIC/anon. Revoke first, then grant exactly the original set.
revoke execute on function public.create_sale(uuid, uuid, jsonb, jsonb, text, uuid, uuid) from public, anon;
grant execute on function public.create_sale(uuid, uuid, jsonb, jsonb, text, uuid, uuid) to authenticated, service_role;
