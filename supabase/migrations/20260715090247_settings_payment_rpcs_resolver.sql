-- Settings S2b: record_customer_payment + record_supplier_payment resolve their cash/bank leg via
-- resolve_payment_account (was 'case p_bank_account_id is not null then 1010 else 1000') — sole resolver,
-- honours the method's mapped bank account's actual GL code, not a hardcoded 1010. Unmapped → 1000 (unchanged).

CREATE OR REPLACE FUNCTION public.record_customer_payment(p_customer_id uuid, p_invoice_id uuid, p_method payment_method_enum, p_amount numeric, p_reference character varying, p_bank_account_id uuid, p_notes text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  v_debit_code := public.resolve_payment_account(v_t, p_method::text, p_bank_account_id);  -- S2b: sole resolver (was 1010/1000 hardcode)
...
begin
  ...
  perform public.post_journal(
    ...
    jsonb_build_array(
      ...
      jsonb_build_object('account_code', public.resolve_payment_account(v_t, p_method::text, p_bank_account_id), 'credit', round(p_amount,4))),  -- S2b: sole resolver (was 1010/1000 hardcode)
    current_date, v_pay_id, false);
