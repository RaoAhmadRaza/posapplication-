-- C5 — Parts output tax. Removes the R4 simplification (parts posted tax-free).
-- Parts lines now carry output tax mirroring create_sale's line-tax math EXACTLY:
-- EXCLUSIVE, tax = round(taxable * tax_pct/100, 4). create_sale takes tax_pct from
-- the caller line (the POS client reads products.tax_rate); here — no caller line —
-- we resolve the basis server-side from products.tax_rate. create_sale does NOT
-- honor tax_inclusive (exclusive only), and parts sell = cost*markup is inherently
-- pre-tax, so exclusive is correct. line_total stays EXCLUSIVE (matches the labour
-- line convention); tax rides in tax_amount and sums into v_tax → 2100 Output Tax.
-- COGS legs (5000/1200 at captured cost) UNCHANGED — tax never touches COGS.
create or replace function public.close_repair_job(p_repair_id uuid, p_labour_price numeric, p_labour_tax_pct numeric, p_parts_markup jsonb, p_paid_amount numeric, p_warranty_days integer, p_signature_url character varying)
 returns jsonb
 language plpgsql
 security definer
...
declare
  ...
  v_part_tax_pct numeric; v_part_line_tax numeric;
...
begin
  ...
  -- Output tax mirrors create_sale (EXCLUSIVE): tax_pct = products.tax_rate, tax = round(sell*qty*pct/100, 4).
  for v_part in select rp.product_id, rp.qty, rp.unit_cost, rp.total_cost from repair_parts rp where rp.repair_id=p_repair_id loop
    ...
    select coalesce(tax_rate,0) into v_part_tax_pct from products where id=v_part.product_id and tenant_id=v_t and deleted_at is null;
    v_part_tax_pct := coalesce(v_part_tax_pct, 0);
    v_part_line_tax := round(v_part_sell * v_part.qty * v_part_tax_pct/100.0, 4);
    v_parts_tax := v_parts_tax + v_part_line_tax;
    insert into invoice_items (invoice_id, product_id, variant_id, imei_id, description, qty, unit_price,
      cost_price, discount_pct, discount_amount, tax_pct, tax_amount, line_total, profit)
    values (v_inv, v_part.product_id, null, null, 'Part', v_part.qty, v_part_sell,
      v_part.unit_cost, 0, 0, v_part_tax_pct, v_part_line_tax, round(v_part_sell*v_part.qty,4), round((v_part_sell-v_part.unit_cost)*v_part.qty,4));
  end loop;
  ...
  -- lines carry account_code (string); post_journal resolves via acct_id. Shape matches create_sale.
  if v_paid > 0 then v_lines := v_lines || jsonb_build_array(jsonb_build_object('account_code','1000','debit',v_paid,'credit',0)); end if;
  if v_balance > 0 then v_lines := v_lines || jsonb_build_array(jsonb_build_object('account_code','1100','debit',v_balance,'credit',0)); end if;
  if coalesce(p_labour_price,0) > 0 then v_lines := v_lines || jsonb_build_array(jsonb_build_object('account_code','4200','debit',0,'credit',round(p_labour_price,4))); end if;
  if v_part_sell_total > 0 then v_lines := v_lines || jsonb_build_array(jsonb_build_object('account_code','4000','debit',0,'credit',v_part_sell_total)); end if;
  if v_tax > 0 then v_lines := v_lines || jsonb_build_array(jsonb_build_object('account_code','2100','debit',0,'credit',v_tax)); end if;
  if v_parts_cost > 0 then
    v_lines := v_lines || jsonb_build_array(
      jsonb_build_object('account_code','5000','debit',v_parts_cost,'credit',0),
      jsonb_build_object('account_code','1200','debit',0,'credit',v_parts_cost));
  end if;
