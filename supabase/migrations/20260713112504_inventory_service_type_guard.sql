-- Inventory hardening: SERVICE products are non-stock by definition. post_stock_movement is the sole
-- chokepoint for every stock write (GRN, count, transfer, sale, repair), so the guard goes HERE — a
-- picker/query filter is UX only, bypassable by a direct RPC call or the bulk importer. Whole-body
-- create-or-replace (carries the R3 REPAIR_USE branch verbatim); only change vs 101141 is the SERVICE
-- reject right after the tenant check, before any operation branch or ledger write.
...
begin
  ...
  -- SERVICE products are non-stock by definition: reject any stock movement of one, on every path.
  if (select type from public.products where id = p_product_id) = 'SERVICE' then
    raise exception 'ERR_SERVICE_NOT_STOCKED';
  end if;

  -- operation-aware permission gate (R1 + R3 repair)
  ...
  elsif p_operation_type = 'REPAIR_USE' then
    if not (public.auth_has_permission('repair','create') or public.auth_has_permission('repair','update')) then
      raise exception 'permission denied' using errcode='42501';
    end if;
  else
