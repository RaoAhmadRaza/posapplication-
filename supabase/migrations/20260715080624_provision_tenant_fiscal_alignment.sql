-- Fix: provision_tenant seeded its fiscal period with date_trunc('year') → an ANNUAL Jan1–Dec31 bucket,
-- while the live model (enforced by current_fiscal_period, the SOLE period creator) is MONTHLY. New tenants
-- ran annual periods; existing tenants ran monthly → granularity divergence (introduced in P2).
-- Fix = reuse the sole creator instead of copying its logic, so the model can never diverge again.
-- current_fiscal_period is VOLATILE + SECURITY DEFINER (verified) — safe to call for its INSERT side effect.
-- EXACTLY ONE block changed (===== 4. FISCAL PERIOD =====); everything else byte-for-byte from the seed fix.
