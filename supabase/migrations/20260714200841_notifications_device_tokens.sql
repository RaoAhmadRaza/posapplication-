...
create index if not exists ix_device_tokens_user on public.device_tokens(user_id) where is_active;

alter table public.device_tokens enable row level security;
drop policy if exists "dt own" on public.device_tokens;
create policy "dt own" on public.device_tokens for all to authenticated
