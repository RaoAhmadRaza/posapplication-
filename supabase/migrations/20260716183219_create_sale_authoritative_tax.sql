-- D6: server-authoritative tax (sign-off #2). GATED ON V0: V0 confirmed pre-launch (1 tenant with
-- invoices, 5 total, 7 payments summing 7,750 PKR, all test-scale; sync_outbox empty) — this MOVES
-- INVOICE TOTALS (p_tax_pct omitted: null->0 becomes null->17%), so it was priced against real launch
-- risk before touching it. Free today; would have been expensive after a real launch.
--
-- tax_rules is the §8 "decorative" finding: seeded, full CRUD UI (M12), read by no RPC. M12 made it
-- authoritative at DATA-ENTRY time only — the UI reads it — while create_sale kept trusting whatever
-- tax_pct the client sent per line, unverified. This closes the other half.
--
-- No caller override kept: D0.7 found no legitimate path in create_sale that needs a client-supplied
-- tax_pct (no per-category/product tax variation exists — every seeded tax_rules row is applies_to='ALL').
-- Every line now resolves from the tenant's single is_default+is_active rule, once per sale, not per line.
--
-- No-active-rule case: RAISE, not silent-0. Bug class #7 (next_number's own self-heal-to-junk, fixed in
-- D1) is exactly a silent-default standing in for a real error; tax on real money gets the same treatment.
--
-- Third live rewrite of create_sale (after D4 idempotency guard). ONE RPC, ONE MIGRATION. Body is
-- byte-identical to the fresh D0.7 dump except: (1) new v_tax_rate/v_tax_mode declarations, (2) a tax-rule
-- resolution block inserted before next_number (fail fast before burning a document number — same
-- reasoning as the idempotency guard's placement), (3) v_tp now assigned from v_tax_rate instead of the
-- caller's payload. Change nothing else.
