-- Settings S2a: create_sale books each payment to its resolved GL account (split tender → one debit/account).
-- ONLY the GL payment-debit shape changed; revenue/tax/COGS legs, stock, IMEI, session guard byte-for-byte.
-- Unmapped methods resolve to '1000' → cash-only tenants unchanged (regression-gated).
