-- 20260617122619_migrate_import_numfix.sql
-- Fix 22P02: wrap every (e->>'col')::numeric/int/boolean/enum cast with nullif(..,'')
-- so empty CSV cells ("") become NULL, then coalesce/default as appropriate.
-- Keeps all pre-filter/dedupe idempotency logic intact.
...
begin
  ...
  with input as (
    select
      ...
      coalesce(nullif(e->>'is_active','')::boolean, true) as act,
      coalesce(nullif(e->>'sort_order','')::int, 0) as so
...
begin
  ...
  ins as (
    ...
    select
      ...
      b.id, c.id,
      coalesce(nullif(_sp,'')::numeric, 0),
      coalesce(nullif(_cp,'')::numeric, 0),
      nullif(_msp,'')::numeric,
      nullif(_wp,'')::numeric,
      coalesce(nullif(_uom, ''), 'PCS'),
      coalesce(nullif(_tax,'')::numeric, 0),
      coalesce(nullif(_rp,'')::int, 0),
      coalesce(nullif(_ia,'')::boolean, true),
...
begin
  ...
  for r in select * from jsonb_array_elements(p_rows) loop
    ...
    begin
      ...
      v_qty := coalesce(nullif(r->>'qty_on_hand','')::numeric, 0);
      v_cost := coalesce(nullif(r->>'avg_cost','')::numeric, 0);
