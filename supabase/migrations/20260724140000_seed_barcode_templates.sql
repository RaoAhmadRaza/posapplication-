-- Seed 3 default barcode LABEL templates (CODE128 / EAN13 / QR) so a new tenant has
-- ready-to-print labels for all three formats WITHOUT hand-writing Layout JSON. Before this,
-- barcode_templates was never provisioned (only sms/email templates were) — a fresh tenant had
-- zero rows, forcing a non-technical user to author raw JSON in the "New template" form.
--
-- Adds section 8 to provision_tenant() (idempotent, name-guarded — the table has NO unique
-- constraint) and backfills every EXISTING tenant that is missing them. layout_json keys
-- {showName,showBarcode,showPrice} match exactly what LabelPdfService reads. is_default is only
-- set on CODE128 and only when the tenant has no default yet (never creates a second default).
-- EAN13 needs a 13-digit code; LabelPdfService falls back to CODE128 rendering otherwise (no crash).
...
begin
  ...
  -- ===== 8. BARCODE LABEL TEMPLATES (3) — one per supported format, ready to print. =====
  -- Table has no unique constraint → guard on (tenant_id, name). is_default only on CODE128, and
  -- only when the tenant has no default yet (the not-exists prevents a second default row).
  insert into barcode_templates (tenant_id, name, format, width_mm, height_mm, layout_json, is_default)
  select p_tenant_id, v.name, v.format, v.w, v.h,
         '{"showName":true,"showBarcode":true,"showPrice":true}'::jsonb,
         v.is_default::boolean
           and not exists (select 1 from barcode_templates d
                             where d.tenant_id=p_tenant_id and d.is_default and d.deleted_at is null)
  from (values
    ('Standard Barcode (CODE128)','CODE128',50,25,'true'),
    ('Retail EAN-13','EAN13',50,30,'false'),
    ('QR Label','QR',40,40,'false')
  ) as v(name,format,w,h,is_default)
  where not exists (select 1 from barcode_templates bt
                      where bt.tenant_id=p_tenant_id and bt.name=v.name and bt.deleted_at is null);

  return public.verify_tenant_provisioning(p_tenant_id);
end; $function$;

-- ===== BACKFILL existing tenants (idempotent, same guards as section 8). =====
insert into barcode_templates (tenant_id, name, format, width_mm, height_mm, layout_json, is_default)
select t.id, v.name, v.format, v.w, v.h,
       '{"showName":true,"showBarcode":true,"showPrice":true}'::jsonb,
       v.is_default::boolean
         and not exists (select 1 from barcode_templates d
                           where d.tenant_id=t.id and d.is_default and d.deleted_at is null)
from tenants t
cross join (values
  ('Standard Barcode (CODE128)','CODE128',50,25,'true'),
  ('Retail EAN-13','EAN13',50,30,'false'),
...
where not exists (select 1 from barcode_templates bt
                    where bt.tenant_id=t.id and bt.name=v.name and bt.deleted_at is null);
