-- R3 File 1: redefine post_stock_movement (live RPC every SALE/GRN calls) to accept REPAIR_USE on
-- repair perms. ONLY change vs the live body: the elsif REPAIR_USE branch in the operation-aware gate.
-- Everything else is byte-for-byte the current prod definition. Isolated in its own migration so a fix
-- to this central RPC never churns the new parts functions (lesson: one live-RPC redefine per file).
