-- disburse_salary_advance: pays an employee an advance now; recovered later via payroll (H5).
-- GL: Dr 1150 Employee Advances / Cr 1000 Cash. Creates the salary_advances row (balance=amount).
create or replace function public.disburse_salary_advance(
  p_employee_id uuid, p_amount numeric, p_recovery_amount numeric, p_pay_account varchar, p_notes text
...
begin
  ...
  select public.post_journal(v_branch, 'SALARY_ADVANCE', gen_random_uuid(), 'Salary advance', v_lines, current_date, null, false) into v_je;
