-- Fix: close_repair_job (migration 102146) built journal lines with the wrong key —
-- 'account_id' pre-resolved via acct_id() — but post_journal reads each line by 'account_code'
-- (v_line->>'account_code' -> acct_id). Every close raised ERR_ACCOUNT_NOT_FOUND: <NULL>.
-- Only the GL line-building block changes: 'account_id',acct_id(v_t,'<code>') -> 'account_code','<code>',
-- matching the canonical shape used by create_sale and the other 5 money-path hooks. Body otherwise verbatim.
