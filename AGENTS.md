...
lessons learned. No feature is permanently off-limits — the project is built out fully over
time, one vertical slice at a time.
...
and widgets — inspect before writing.
...
3. Write code (normal prose inside code, comments minimal).
4. `flutter analyze` → fix.
